...
export class Pacientes {
    ...
    getPacientes
    constructor(){
        // console.log(getPacientes,"getPacientes");
    ...
    renderCitas(paciente,medicos,buttonMaster,renderGen){
        ...
        const reload =(prevData,buttonReload,medicos,renderCitas,paciente,renderGen)=>{
            console.log(renderGen,"renderGen")
            this.container = document.querySelector("#pacientes");
            ...
            const newPaciente = prevData.find(p=>p.idPaciente === paciente.idPaciente)
            const citas = this.renderCitas(newPaciente,medicos,buttonReload)
            // console.log(citas)
            bodyModal.replaceChildren(citas)
            // this.modal.cerrarModal()
            // this.getPacientes()
            // console.log("RELOAD",prevData,this.render,this.renderCitas);
        ...
        citasPrevContainer.className = "d-flex flex-wrap justify-content-center gap-2"
        paciente.citas.sort((a,b) => new Date(a.fecha) - new Date(b.fecha)).forEach((cita) => {
        //   console.log(paciente);
          // console.log(medicos.find(m=> m.turnosId.includes(cita.id_turno)));
          const medico = medicos.find((m) =>
            m.turnosId.includes(cita.id_turno)
          );
    ...
    render(pacientes,medicos,buttonReload,renderGen){
        ...
        containerPrevCards.className =" d-flex flex-wrap justify-content-center gap-2";
        ...
        buttonMaster.addEventListener("click",()=>{
            ...
            const btn = document.querySelector(`[id="button_${paciente.idPaciente}"]`);
            // console.log(btn)
